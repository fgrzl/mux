...
func currentCookieNames(c RouteContext) cookiekit.CookieNames {
	if c == nil {
		return cookiekit.DefaultCookieNames()
	}
	if value, ok := c.GetService(cookiekit.ServiceKeyCookieNames); ok {
		return cookiekit.ResolveCookieNames(value)
	}
	return cookiekit.DefaultCookieNames()
}
...
func SignOutWithOptions(c RouteContext, redirectUrl string, opts ...cookiekit.CookieOption) {
	...
	cookieNames := currentCookieNames(c)
	ClearCookieWithOptions(c, cookieNames.AppSession, opts...)
